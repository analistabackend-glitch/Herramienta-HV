...
def _construir_hoja2(writer, resumen_tercer_filtro: list):
    ...
    for row_idx, fila in enumerate(filas, start=2):
        razon_exp = str(fila.get("Razón Experiencia", ""))
        resumen_txt = str(fila.get("Resumen", ""))

        # Prioridad de color:
        # 1. Error de procesamiento → naranja oscuro
        # 2. No es HV → naranja claro
        # 3. Experiencia en 0 meses → amarillo
        # 4. Color normal por clasificación
        if "REVISAR MANUALMENTE — Error al procesar" in razon_exp:
            color = "FFD580"   # naranja más marcado para errores de procesamiento
        elif "DOCUMENTO NO CORRESPONDE" in razon_exp:
            color = COLOR_NARANJA_ADV
        elif "REVISAR MANUALMENTE — hay al menos una experiencia con duración 0" in razon_exp \
             or "REVISAR MANUALMENTE — hay al menos una experiencia con duración 0" in resumen_txt:
...
def generar_excel_unificado(
    ...
    conteos = {
        ...
        "opcionados"    : sum(1 for r in resumen_tercer_filtro
                              if r.get("Clasificación") == "Opcionales"),
